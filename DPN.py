...
def get_config():
    config = {
        ...
        "epoch": 100,
        "test_map": 10,
        "save_path": "savepr/DPN1",
        #"device":torch.device("cpu"),
        "device": torch.device("cuda:3"),
        "bit_list": [16],
        # "npsavetxt": "cifar1048DPNdateset_binary",
    }
...
class DPNLoss(torch.nn.Module):
    ...
    def forward(self, u, y, ind, config):
        ...
        # print(u.shape)torch.Size([64, 32])
        # print(t.shape)torch.Size([64, 32])
        # exit()
...
def train_val(config, bit):
    ...
    net = config["net"](bit).to(device)#3.20注释，修改transform
    # net = config["net"](bit).to(device)#3.20注释，修改transform
    ...
    for epoch in range(config["epoch"]):
        ...
        if (epoch + 1) % config["test_map"] == 0:
            ...
            trn_binary, trn_label = compute_result(dataset_loader, net, device=device)
            # if not os.path.isdir('SNE/'+config["dataset"]+'/'+config["info"]+'/'+str(bit)):
                # os.makedirs('SNE//'+config["dataset"]+'//'+config["info"]+'//'+str(bit))
            # np.savetxt('SNE/'+config["dataset"]+'/'+config["info"]+'/'+str(bit)+'/'+'dateset_binary.txt',trn_binary,fmt='%d')
            # np.savetxt('SNE/'+config["dataset"]+'/'+config["info"]+'/'+str(bit)+'/'+'dateset_label.txt',trn_label,fmt='%d')
...
if __name__ == "__main__":
    ...
    for bit in config["bit_list"]:
        # config["pr_curve_path"] = f"DELETEHASH/PIT/DPNRESNET_{config['dataset']}_{bit}.json"
